...
AI 开启期间，每个标签页会在主进程内存中维护独立的短期上下文，包括近期命令、工作目录、退出码和受限输出，用于帮助模型理解后续意图。关闭 AI、关闭标签页或 Shell 退出时会立即清空该上下文，不会写入磁盘，也不会清除 Shell 自己的命令历史。
...
AI 开关按标签页隔离。关闭 AI 会取消该标签页尚未完成的模型请求，不影响其他标签页。

## 8. 数据与安全边界

- AI 关闭时不发起模型请求，并取消该会话未完成的请求。
- 只向模型发送有限的 Shell、系统、工作目录、用户意图、失败命令或当前标签页命令上下文。
- 临时 AI 上下文最多保留 12 条近期记录、200 行和 32 KB，写入内存前会脱敏，并且不会持久化到磁盘。
- 关闭 AI 或标签页会清空对应的临时 AI 上下文，但不会删除 Shell 原生命令历史。
